    ...
    public class PkixNameConstraintValidator
    {
        ...
        private int HashCollection(ICollection c)
        {
            ...
            foreach (Object o in c)
            {
                ...
                {
                    var s = (ArraySegment<byte>)o;
                    hash += Arrays.GetHashCode(s.Array, s.Offset, s.Count);
                }
                else
                {
                    hash += o.GetHashCode();
        ...
        private bool SpecialEquals(Object o1, Object o2)
        {
            ...
            ArraySegment<byte>? s1 = default, s2 = default;

            if (o1 is byte[])
            {
                var array = (byte[])o1;

                s1 = new ArraySegment<byte>(array, 0, array.Length);
            }
            else if (o1 is ArraySegment<byte>)
            {
                s1 = (ArraySegment<byte>)o1;
            }
            if (o2 is byte[])
            {
                var array = (byte[])o2;

                s2 = new ArraySegment<byte>(array, 0, array.Length);
            }
            else if (o2 is ArraySegment<byte>)
            {
                s2 = (ArraySegment<byte>)o2;
            }

            if (s1 != null && s2 != null)
            {
                return Arrays.AreEqual(s1.Value.Array, s1.Value.Offset, s1.Value.Offset + s1.Value.Count,
                    s2.Value.Array, s2.Value.Offset, s2.Value.Offset + s2.Value.Count);
            }
            else
            {
                return o1.Equals(o2);
